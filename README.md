...
A standalone Next.js App Router application for COMQ CIA, with Spanish at `/` and English at `/en`. The homepage includes the shared navigation, footer, company introduction and equipment sales, rentals and spare-parts sections. It uses server components, typed dictionaries and CSS Modules.

Each homepage service section links to its own page: `/venta`, `/alquiler` and `/repuestos`, with matching `/en` routes. Header service links open the same pages. Each service page currently contains only its translated title, with the shared header and footer. The Spanish homepage service copy is preserved from the original page and translated into English. Local SVG image placeholders appear below the service headings until the original images are supplied.
...
`lib/site` centralizes the canonical origin (`https://www.comqcia.com`), company identity, WhatsApp and LinkedIn contact links, localized metadata and safe JSON-LD serialization. The sitemap contains the homepage and three service pages in both languages; it deliberately omits invented modification dates. Each service route supplies its own title and description to the metadata helper. Social metadata is text-based. The company and website schemas use the supplied public facts.
